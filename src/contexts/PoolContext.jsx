...
export function PoolContextProvider({children}) {
        ...
        if (account) {
            try {
                ...
                const stakingEpochDuration =
                    (parseInt(stakingEpochEndBlock) - parseInt(currentBlock)) * config.secPerBlock;
    ...
    const getSyxData = useCallback(async () => {
        if (account) {
            try {
                const syxContract = new Contract(config.newSyx, config.erc20ABI, ethersProvider);
        ...
        async (type, amount) => {
            if (account) {
                ...
                try {
                    ...
                    const syxContract = new Contract(config.newSyx, config.syxABI, signer);
                    const oldSyxContract = new Contract(oldSyxAddress, config.erc20ABI, signer);
                    const allowance = await oldSyxContract.allowance(account, config.newSyx);
                    if (parseFloat(allowance) < parseFloat(amount)) {
                        const tx = await oldSyxContract.approve(config.newSyx, amount);
                        await tx.wait();
                    }
                    const tx2 = await syxContract.exchangeSyx(oldSyxAddress, amount.toString());
                    await tx2.wait();
                    getSyxData();
                    getOldSyxData();
                } catch (error) {
