...
class Store {
    ...
    getBalancesPerpetual = async () => {
        ...
        try {
            ...
            syxBalance = await this.getErc20Balance(
                web3,
                config.syx,
                18,
                account
                ...
                async.parallel(
                    ...
                    (err, data) => {
                        ...
                        if (pool.type === "seed") {
                            ...
                            pool.totalBalanceForSyx =
                                parseFloat(data[7].erc20Balance) /
                                    parseFloat(pool.price) +
                        ...
                        pool.maxErc20In =
                            parseFloat(data[7].maxIn) *
                            parseFloat(data[7].erc20Balance);
                        pool.maxSyxIn =
                            parseFloat(data[7].maxIn) *
                            parseFloat(data[7].erc20Balance2);
                        pool.maxErc20Out =
                            parseFloat(data[7].maxOut) *
                            parseFloat(data[7].erc20Balance);
                        pool.maxSyxOut =
                            parseFloat(data[7].maxOut) *
                            parseFloat(data[7].erc20Balance2);
        ...
        if (asset.type === "seed") {
            ...
            try {
                ...
                if (asset.erc20Address2 === asset.rewardsAddress) {
                    ...
                    callback(null, {
                        erc20Balance: toStringDecimals(balance, asset.decimals),
                        erc20Balance2: toStringDecimals(
                            balance2,
                            asset.erc20Decimals2
                        )
                    });
                }
            } catch (ex) {
    ...
    _calculateTokenPrice = async (
        ...
        try {
            ...
            if (type === "sell") {
                if (
                    tokenIn === asset.erc20Address &&
                    asset.erc20Decimals !== 18
                ) {
                ...
                if (
                    ...
                    const inAmount = parseFloat(
                        parseFloat(
                            balanceIn / Number(`1e+${asset.erc20Decimals}`)
                        ) + amount
                    );
                    ...
                    const inAmount = parseFloat(
                        parseFloat(
                            balanceIn / Number(`1e+${asset.erc20Decimals2}`)
                        ) + amount
                    );
                    calcInAmount = parseInt(
                ...
                if (
                    tokenOut === asset.erc20Address &&
                    asset.erc20Decimals !== 18
                ) {
                    const outAmount =
                        parseFloat(
                            balanceOut / Number(`1e+${asset.erc20Decimals}`)
                        ) -
                        parseFloat(
                            tokenAmountOut / Number(`1e+${asset.erc20Decimals}`)
                        );
                    ...
                    const outAmount =
                        parseFloat(
                            balanceOut / Number(`1e+${asset.erc20Decimals2}`)
                        ) -
                        parseFloat(
                            tokenAmountOut /
                                Number(`1e+${asset.erc20Decimals2}`)
                        );
                    calcOutAmount = parseInt(
                ...
                if (
                    ...
                    const inAmount =
                        ...
                        parseFloat(
                            tokenAmountIn / Number(`1e+${asset.erc20Decimals}`)
                        );
                    calcInAmount = parseInt(
                        inAmount * Number(`1e+${asset.erc20Decimals}`)
                    ).toLocaleString("fullwide", {useGrouping: false});
                    ...
                    const inAmount =
                        ...
                        parseFloat(
                            tokenAmountIn / Number(`1e+${asset.erc20Decimals2}`)
                        );
                    calcInAmount = parseInt(
                        inAmount * Number(`1e+${asset.erc20Decimals2}`)
                    ).toLocaleString("fullwide", {useGrouping: false});
            ...
            try {
                let [
                    ...
                    [
                        ...
                        bptContract.methods.getNormalizedWeight(
                            asset.erc20Address2
                        ).call,
                        bptContract.methods.getNormalizedWeight(
                            asset.erc20Address
                        ).call,
                        bptContract.methods.getBalance(asset.erc20Address2)
                            .call,
                ...
                callback(null, {
                    ...
                    weight:
                        parseInt(
                            toStringDecimals(weight1 * 100, asset.decimals)
                        ) +
                        ":" +
                        parseInt(
                            toStringDecimals(weight2 * 100, asset.decimals)
                        ),
                    erc20Balance2: toStringDecimals(
                        erc20Balance2,
                        asset.erc20Decimals2
                    ),
                    erc20Balance: toStringDecimals(
                        erc20Balance,
                        asset.erc20Decimals
                    ),
                    totalSupply: toStringDecimals(totalSupply, asset.decimals)
