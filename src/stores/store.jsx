...
class Store {
      ...
      (pool, callback) => {
        async.parallel(
          [
            //0
            (callbackInner) => {
              this._getStakeAmount(web3, pool, account, callbackInner);
            },
            //1
            (callbackInner) => {
              this._getRewardsAvailable(web3, pool, account, callbackInner);
            },
            //2
            (callbackInner) => {
              this._getERC20TokenPrice(web3, pool, account, callbackInner);
            },
            //3
            (callbackInner) => {
              this._getTokenBalance(web3, pool, account, callbackInner);
            },
            //4
            (callbackInner) => {
              this._getRewardRate(web3, pool, account, callbackInner);
            },
            //5
            (callbackInner) => {
              this._getAllocPoint(web3, pool, account, callbackInner);
            },
            //6
            (callbackInner) => {
            ...
            //7
            (callbackInner) => {
              this._getBptInfo(web3, pool, account, callbackInner);
            },
          ],
          (err, data) => {
            if (err) {
            ...
            if (poolData[i] && poolData[i].type === "seed") {
              for (let j = 0; j < poolData.length; j++) {
                if (poolData[j] && poolData[j].id === "SYX2/VLX") {
                  ...
                  poolData[i].rewardApr = (poolData[i].totalBalanceForSyx > 0
                    ? ((parseFloat(poolData[i].rewardRate) * blocksPerYear) /
                        poolData[i].totalBalanceForSyx) *
                      100
                    : 0
                  ).toFixed(1);
    ...
    if (!entryContractAddress) {
      ...
      try {
        let userInfo = await contract.methods
          ...
          .call();
        callback(null, toStringDecimals(userInfo.amount, asset.decimals));
      } catch (ex) {
        return callback(ex);
      }
    }
    ...
    if (asset.type === "seed") {
      ...
      try {
        ...
        if (asset.erc20Address2 === asset.rewardsAddress) {
          ...
          callback(null, {
            erc20Balance: toStringDecimals(balance, asset.decimals),
            erc20Balance2: toStringDecimals(balance2, asset.erc20Decimals2),
          });
        }
      } catch (ex) {
        return callback(ex);
      ...
      callback(null, {
        erc20Balance: toStringDecimals(balance, asset.erc20Decimals),
        erc20Balance2: toStringDecimals(balance2, asset.erc20Decimals2),
      });
    }
  };
  ...
  _getRewardRate = async (web3, asset, account, callback) => {
    let erc20Contract = new web3.eth.Contract(asset.poolABI, asset.poolAddress);
    try {
      const curBlockNumber = await web3.eth.getBlockNumber();
      let [rate, bonusEndBlock, startBlock, endBlock] = await makeBatchRequest(
        web3,
        [
      ...
      if (parseFloat(curBlockNumber) >= parseFloat(endBlock)) {
        ...
        const bonusMultiplier = await erc20Contract.methods
          .BONUS_MULTIPLIER()
          .call();
        callback(
          null,
          toStringDecimals(parseFloat(rate) * parseFloat(bonusMultiplier), 18)
        );
      } else {
  ...
  _getAllocPoint = async (web3, asset, account, callback) => {
    let erc20Contract = new web3.eth.Contract(asset.poolABI, asset.poolAddress);
    try {
      let [totalAllocPoint, poolInfo] = await makeBatchRequest(
        web3,
        [
      ...
      if (type === "sell") {
        ...
        if (tokenIn === asset.erc20Address && asset.erc20Decimals !== 18) {
          const inAmount = parseFloat(
            parseFloat(balanceIn / Number(`1e+${asset.erc20Decimals}`)) + amount
          );
          calcInAmount = parseInt(
            inAmount * Number(`1e+${asset.erc20Decimals}`)
          ).toLocaleString("fullwide", { useGrouping: false });
        } else if (
          tokenIn === asset.erc20Address2 &&
          asset.erc20Decimals2 !== 18
          ...
          const inAmount = parseFloat(
            parseFloat(balanceIn / Number(`1e+${asset.erc20Decimals2}`)) +
              amount
          );
          calcInAmount = parseInt(
            inAmount * Number(`1e+${asset.erc20Decimals2}`)
          ).toLocaleString("fullwide", { useGrouping: false });
        } else {
          calcInAmount = web3.utils.toWei(
            parseFloat(web3.utils.fromWei(balanceIn, "ether")) + amount + "",
            "ether"
          );
        }
        ...
        if (tokenOut === asset.erc20Address && asset.erc20Decimals !== 18) {
          const outAmount =
            parseFloat(balanceOut / Number(`1e+${asset.erc20Decimals}`)) -
            parseFloat(tokenAmountOut / Number(`1e+${asset.erc20Decimals}`));
          calcOutAmount = parseInt(
            outAmount * Number(`1e+${asset.erc20Decimals}`)
          ).toLocaleString("fullwide", { useGrouping: false });
          tradePrice =
            tokenAmountOut / Number(`1e+${asset.erc20Decimals}`) / amount;
        } else if (
          tokenOut === asset.erc20Address2 &&
          asset.erc20Decimals2 !== 18
        ) {
          const outAmount =
            parseFloat(balanceOut / Number(`1e+${asset.erc20Decimals2}`)) -
            parseFloat(tokenAmountOut / Number(`1e+${asset.erc20Decimals2}`));
          calcOutAmount = parseInt(
            outAmount * Number(`1e+${asset.erc20Decimals2}`)
          ).toLocaleString("fullwide", { useGrouping: false });
          tradePrice =
            tokenAmountOut / Number(`1e+${asset.erc20Decimals2}`) / amount;
        } else {
          calcOutAmount = web3.utils.toWei(
            parseFloat(web3.utils.fromWei(balanceOut, "ether")) -
              parseFloat(web3.utils.fromWei(tokenAmountOut, "ether")) +
              "",
            "ether"
          );
          //Trading price
        ...
        if (tokenIn === asset.erc20Address && asset.erc20Decimals !== 18) {
          const inAmount =
            parseFloat(balanceIn / Number(`1e+${asset.erc20Decimals}`)) +
            parseFloat(tokenAmountIn / Number(`1e+${asset.erc20Decimals}`));
          calcInAmount = parseInt(
            inAmount * Number(`1e+${asset.erc20Decimals}`)
          ).toLocaleString("fullwide", { useGrouping: false });
          tradePrice =
            (amount * Number(`1e+${asset.erc20Decimals}`)) / tokenAmountIn;
        } else if (
          tokenIn === asset.erc20Address2 &&
          asset.erc20Decimals2 !== 18
        ) {
          const inAmount =
            parseFloat(balanceIn / Number(`1e+${asset.erc20Decimals2}`)) +
            parseFloat(tokenAmountIn / Number(`1e+${asset.erc20Decimals2}`));
          calcInAmount = parseInt(
            inAmount * Number(`1e+${asset.erc20Decimals2}`)
          ).toLocaleString("fullwide", { useGrouping: false });
          tradePrice =
            (amount * Number(`1e+${asset.erc20Decimals2}`)) / tokenAmountIn;
        } else {
          calcInAmount = web3.utils.toWei(
            parseFloat(web3.utils.fromWei(balanceIn, "ether")) +
              parseFloat(web3.utils.fromWei(tokenAmountIn, "ether")) +
              "",
            "ether"
          );
        ...
        if (tokenOut === asset.erc20Address && asset.erc20Decimals !== 18) {
          ...
          calcOutAmount = web3.utils.toWei(
            parseFloat(web3.utils.fromWei(balanceOut, "ether")) - amount + "",
            "ether"
          );
        }
      ...
      try {
        ...
        if (asset.erc20Decimals != asset.erc20Decimals2) {
          if (asset.erc20Decimals < asset.erc20Decimals2) {
            ...
            decimals =
              asset.erc20Decimals + asset.erc20Decimals - asset.erc20Decimals2;
          }
        }
    ...
    if (!entryContractAddress) {
      ...
      if (token === asset.erc20Address) {
        if (asset.erc20Decimals !== 18) {
          amountToSend = (amount * Number(`1e+${asset.erc20Decimals}`)).toFixed(
            0
          );
        }
      } else if (token === asset.erc20Address2) {
  ...
  _callWithdraw = async (asset, account, token, amount, callback) => {
    ...
    if (!entryContractAddress) {
      ...
      let gasLimit;
      try {
        gasLimit = await yCurveFiContract.methods
          .withdraw(...args)
          .estimateGas({
            from: account.address,
            gasPrice: web3.utils.toWei(await this._getGasPrice(), "gwei"),
  ...
  _callTrade = async (
    ...
    const web3 = await this.getWeb3();
    const yCurveFiContract = new web3.eth.Contract(asset.abi, asset.address);
    ...
    if (token === asset.erc20Address) {
      ...
      if (asset.erc20Decimals2 !== 18) {
        amountToSend = (amount * Number(`1e+${asset.erc20Decimals2}`)).toFixed(
          0
        );
      }
    } else {
    ...
    if (asset.type === "swap-native" && asset.erc20Address === token) {
      ...
      yCurveFiContract.methods
        .swapWTokenAmountIn(token2, "0", web3.utils.toWei(price + "", "ether"))
        .send({
          value: amountToSend,
          from: account.address,
          gasPrice: web3.utils.toWei(await this._getGasPrice(), "gwei"),
    ...
    } else if (asset.type === "swap-native" && asset.erc20Address !== token) {
